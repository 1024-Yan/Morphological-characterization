...
for type_, group in df.groupby('Group'):  
    ...
    plt.scatter(group.iloc[:, 4], group.iloc[:, 5], color=color, alpha=0.8, label=type_)  
...
plt.xlabel('Height of hypothallia (μm)')
plt.ylabel('Thallus thickness (μm)')
plt.title('Scatter Plot of thallus thickness')
plt.savefig('thallus characters.svg', format='svg')